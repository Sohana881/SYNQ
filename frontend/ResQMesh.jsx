...
import {
  ...
  Smartphone, Monitor, WifiOff, Wifi, Mic, MicOff, ArrowDown, Brain, Construction, ArrowLeft,
} from "lucide-react";
import {
...
const CRITICAL_CONDITIONS = ["unconscious", "severe bleeding", "cardiac arrest", "not breathing", "chest pain", "head injury", "collapse"];
const CONDITION_KEYWORDS = [
  ...
  "trapped", "seizure", "head injury", "fall", "collapse",
];
...
function calculatePriority(incident) {
  ...
  if (incident.conditions.includes("collapse")) { score += 35; reasons.push("Structural collapse"); }
  ...
  return (
    <nav aria-label="Primary" style={{ width: collapsed ? 60 : 216, flexShrink: 0, borderRight: "1px solid var(--border)", background: "var(--panel)", display: "flex", flexDirection: "column", transition: "width .15s" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "16px 14px", borderBottom: "1px solid var(--border)" }}>
        ...
        {!collapsed && <span className="rqm-mono" style={{ fontWeight: 800, letterSpacing: ".08em", fontSize: "0.95em" }}>SYNCQ</span>}
      </div>
...
function Topbar({ theme, setTheme }) {
  ...
  return (
    <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px", borderBottom: "1px solid var(--border)", background: "var(--bg-raised)" }}>
      <div>
        <div style={{ fontWeight: 800, fontSize: "1.05em", letterSpacing: ".02em" }}>SYNCQ COMMAND CENTER</div>
        <div style={{ fontSize: "0.75em", color: "var(--text-faint)" }}>Emergency intelligence that keeps moving.</div>
      </div>
...
// ---- CommandCenter.jsx ------------------------------------------------------
function SyncQSituation() {
  const { state } = useApp();
  const criticalCount = state.incidents.filter((i) => i.priorityResult?.priority === "CRITICAL").length;
  const blockedCount = state.incidents.filter((i) => i.syncq?.type === "ROAD_BLOCKED").length;
  const collapse = state.incidents.find((i) => i.syncq?.type === "BUILDING_COLLAPSE");
  const roadBlock = state.incidents.find((i) => i.syncq?.type === "ROAD_BLOCKED");

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 12 }}>
        <div className="rqm-panel" style={{ padding: "12px 16px" }}>
          <div style={{ fontSize: "0.72em", color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: ".06em", fontWeight: 700, marginBottom: 6 }}>Network</div>
          <div className="rqm-mono" style={{ display: "flex", alignItems: "center", gap: 6, color: "var(--critical)", fontWeight: 700 }}><WifiOff size={15} /> OFFLINE / DEGRADED</div>
        </div>
        <div className="rqm-panel" style={{ padding: "12px 16px" }}>
          <div style={{ fontSize: "0.72em", color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: ".06em", fontWeight: 700, marginBottom: 6 }}>Active Devices</div>
          <div className="rqm-mono" style={{ fontSize: "1.4em", fontWeight: 700 }}>4</div>
        </div>
        <div className="rqm-panel" style={{ padding: "12px 16px" }}>
          <div style={{ fontSize: "0.72em", color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: ".06em", fontWeight: 700, marginBottom: 6 }}>Critical Incidents</div>
          <div className="rqm-mono" style={{ fontSize: "1.4em", fontWeight: 700, color: "var(--critical)" }}>{criticalCount}</div>
        </div>
        <div className="rqm-panel" style={{ padding: "12px 16px" }}>
          <div style={{ fontSize: "0.72em", color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: ".06em", fontWeight: 700, marginBottom: 6 }}>Blocked Routes</div>
          <div className="rqm-mono" style={{ fontSize: "1.4em", fontWeight: 700, color: "var(--high)" }}>{blockedCount}</div>
        </div>
      </div>
      ...
      {(collapse || roadBlock) && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 12 }}>
          {collapse && (
            <div className="rqm-panel rqm-fade-in" style={{ padding: 16, borderColor: "var(--critical)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                <Badge tone="critical">🚨 Critical Incident</Badge>
              </div>
              <div style={{ fontWeight: 800, fontSize: "1.05em", marginBottom: 6 }}>BUILDING COLLAPSE</div>
              <div style={{ fontSize: "0.82em", color: "var(--text-dim)", lineHeight: 1.6 }}>
                {collapse.syncq?.trapped ?? "?"} trapped · {collapse.syncq?.injured ?? "?"} injured<br />
                📍 {collapse.location?.name || "Location attached"}
              </div>
            </div>
          )}
          {roadBlock && (
            <div className="rqm-panel rqm-fade-in" style={{ padding: 16, borderColor: "var(--high)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                <Badge tone="high">🚧 Road Blocked</Badge>
              </div>
              <div style={{ fontWeight: 800, fontSize: "1.05em", marginBottom: 6 }}>Debris Detected</div>
              <div style={{ fontSize: "0.82em", color: "var(--text-dim)", lineHeight: 1.6 }}>
                Reported by a nearby relayed device.<br />
                📍 {roadBlock.location?.name || "Location attached"}
              </div>
            </div>
          )}
        </div>
      ...
      {collapse && (
        <Panel title="AI Situation Summary" icon={Brain} style={{ borderColor: "var(--accent)" }}>
          <p style={{ fontSize: "0.88em", lineHeight: 1.7, margin: 0 }}>
            Critical incident detected in Sector A. {collapse.syncq?.trapped ?? "Two"} people are trapped and {collapse.syncq?.injured ?? "one"} person is injured.
            {roadBlock ? " The western access route is blocked." : ""} Recommended approach: eastern route.
          </p>
        </Panel>
      )}
    </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 16 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <Panel title="Response Lab" icon={Zap}>
          <p style={{ fontSize: "0.82em", color: "var(--text-dim)", marginBottom: 12 }}>Trigger a live-environment event and watch SYNCQ detect the change, check whether the current plan is still valid, and re-plan if needed.</p>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <span style={{ fontSize: "0.78em", color: "var(--text-faint)" }}>Illustrative demo data — not a scientific claim. Compares a single-LLM baseline against the structured SYNCQ pipeline.</span>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        <Panel title="SYNCQ" icon={Layers}><p style={{ fontSize: "0.82em" }}>Structured pipeline: extraction, priority rules, feasibility, adversarial review, and deterministic verification each run as separate, auditable stages before a human approves.</p></Panel>
      </div>
        ...
        <div style={{ width: "100%", height: 320 }}>
          <ResponsiveContainer>
            <BarChart data={BENCHMARK_DATA} layout="vertical" margin={{ left: 40 }}>
              ...
              <Bar dataKey="resqmesh" name="SYNCQ pipeline" fill="var(--accent)" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </Panel>
...
/* =========================================================================
   SYNCQ FRONT FLOW — Home / Report / Signal
   (src/pages/SyncQHome.jsx, SyncQReport.jsx, SyncQSignal.jsx)
   ========================================================================= */
function parseCasualtyCounts(text) {
  const lower = text.toLowerCase();
  const toNum = (s) => (s ? (WORD_NUMBERS[s] ?? parseInt(s, 10) ?? null) : null);
  const numWord = "(\\d+|one|two|three|four|five|six)";
  const trappedMatch = lower.match(new RegExp(`${numWord}[^.]*?trapped`)) || lower.match(new RegExp(`trapped[^.]*?${numWord}`));
  const injuredMatch = lower.match(new RegExp(`${numWord}[^.]*?injured`)) || lower.match(new RegExp(`injured[^.]*?${numWord}`));
  return { trapped: trappedMatch ? toNum(trappedMatch[1]) : null, injured: injuredMatch ? toNum(injuredMatch[1]) : null };
}
function emergencyTypeLabel(text) {
  const lower = text.toLowerCase();
  if (lower.includes("collapse")) return "BUILDING COLLAPSE";
  if (lower.includes("fire")) return "FIRE";
  if (lower.includes("flood")) return "FLOOD";
  if (lower.includes("accident") || lower.includes("crash")) return "VEHICLE ACCIDENT";
  if (lower.includes("cardiac") || lower.includes("not breathing")) return "MEDICAL EMERGENCY";
  return "EMERGENCY REPORTED";
}

const DEMO_REPORT_TEXT = "There was a building collapse. Two people are trapped and one is injured.";

function SyncQHome({ onReport, onCommand }) {
  return (
    <div className="rqm-root rqm-fade-in" data-theme="dark" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, textAlign: "center" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
        <Radio size={30} style={{ color: "var(--accent)" }} />
        <span className="rqm-mono" style={{ fontWeight: 800, fontSize: "2.2em", letterSpacing: ".06em" }}>SYNCQ</span>
      </div>
      <div style={{ color: "var(--text-dim)", fontSize: "1.05em", marginBottom: 40 }}>Emergency intelligence that keeps moving.</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 14, width: "100%", maxWidth: 340 }}>
        <Button variant="accent" icon={Siren} onClick={onReport} style={{ padding: "16px 20px", fontSize: "1em", justifyContent: "center" }}>REPORT EMERGENCY</Button>
        <Button icon={Gauge} onClick={onCommand} style={{ padding: "16px 20px", fontSize: "1em", justifyContent: "center" }}>COMMAND CENTER</Button>
      </div>
      <div className="rqm-mono" style={{ marginTop: 44, fontSize: "0.78em", color: "var(--low)", display: "flex", alignItems: "center", gap: 6, border: "1px solid var(--low)", padding: "6px 12px", borderRadius: 6 }}>
        <span style={{ width: 7, height: 7, borderRadius: "50%", background: "var(--low)" }} className="rqm-pulse" /> LOCAL NETWORK: READY
      </div>
    </div>
  );
}

function SyncQReport({ onBack, onSignal }) {
  const [text, setText] = useState("");
  const [phase, setPhase] = useState("input"); // input | understanding | result
  ...
  const submit = async () => {
    const raw = text.trim() || DEMO_REPORT_TEXT;
    setPhase("understanding");
    await wait(1100);
    const parsed = extractIncident(raw);
    const priority = calculatePriority(parsed);
    const counts = parseCasualtyCounts(raw);
    setResult({
      raw, priority: priority.priority,
      type: emergencyTypeLabel(raw),
      trapped: counts.trapped, injured: counts.injured,
  ...
  return (
    <div className="rqm-root rqm-fade-in" data-theme="dark" style={{ minHeight: "100vh", padding: 24, display: "flex", justifyContent: "center" }}>
      <div style={{ width: "100%", maxWidth: 480, display: "flex", flexDirection: "column", gap: 18 }}>
        <button onClick={onBack} className="rqm-btn rqm-focus" style={{ background: "none", border: "none", color: "var(--text-dim)", display: "flex", alignItems: "center", gap: 6, cursor: "pointer", padding: 0, alignSelf: "flex-start" }}>
          <ArrowLeft size={16} /> Back
        </button>
        <div>
          <div className="rqm-mono" style={{ fontWeight: 800, fontSize: "1.3em", letterSpacing: ".04em" }}>REPORT EMERGENCY</div>
          <div style={{ color: "var(--text-faint)", fontSize: "0.82em", marginTop: 4 }}>Describe what's happening. SYNCQ will understand it, even offline.</div>
        </div>

        {phase === "input" && (
          <>
            <div className="rqm-panel" style={{ padding: 14 }}>
              <textarea
                value={text}
                onChange={(e) => setText(e.target.value)}
                placeholder={DEMO_REPORT_TEXT}
                rows={5}
                className="rqm-focus"
                style={{ width: "100%", background: "var(--panel-2)", border: "1px solid var(--border)", borderRadius: 8, color: "var(--text)", padding: 12, fontFamily: "var(--font-ui)", fontSize: "0.92em", resize: "vertical" }}
              />
              <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
                {micSupported && (
                  <Button icon={listening ? MicOff : Mic} variant={listening ? "accent" : "default"} onClick={listening ? stopMic : startMic}>
                    {listening ? "Listening…" : "Speak"}
                  </Button>
                )}
                <Button icon={FileWarning} onClick={() => setText(DEMO_REPORT_TEXT)}>Use demo report</Button>
              </div>
            </div>
            <Button variant="accent" icon={Send} onClick={submit} style={{ padding: "14px 20px", justifyContent: "center", fontSize: "0.95em" }}>SUBMIT</Button>
          </>
        )}

        {phase === "understanding" && (
          <div className="rqm-panel rqm-fade-in" style={{ padding: 30, textAlign: "center" }}>
            <RefreshCw size={26} className="rqm-pulse" style={{ color: "var(--accent)", marginBottom: 12 }} />
            <div className="rqm-mono" style={{ letterSpacing: ".08em", color: "var(--text-dim)" }}>UNDERSTANDING EMERGENCY...</div>
          </div>
        )}

        {phase === "result" && result && (
          <div className="rqm-panel rqm-fade-in" style={{ padding: 20, borderColor: "var(--critical)" }}>
            <Badge tone={priorityTone(result.priority)}>🚨 {result.priority}</Badge>
            <div style={{ fontWeight: 800, fontSize: "1.2em", margin: "10px 0" }}>{result.type}</div>
            <div style={{ fontSize: "0.9em", color: "var(--text-dim)", lineHeight: 1.9 }}>
              {result.trapped != null && <div>{result.trapped} people trapped</div>}
              {result.injured != null && <div>{result.injured} injured</div>}
              <div>📍 Location attached</div>
            </div>
            <Button variant="accent" icon={Radio} onClick={() => onSignal(result)} style={{ marginTop: 16, padding: "14px 20px", justifyContent: "center", width: "100%", fontSize: "0.95em" }}>
              CREATE SYNCQ SIGNAL
            </Button>
          </div>
        )}
...
const SIGNAL_STEPS = [
  "EMERGENCY DETECTED",
  "AI UNDERSTANDS",
  "EMERGENCY SIGNAL CREATED",
  "SEARCHING FOR NEARBY DEVICES",
  "NODE FOUND ✓",
  "SIGNAL RELAYED ✓",
];

function SyncQSignal({ onDone }) {
  const [stepIndex, setStepIndex] = useState(0);
  const [relayIndex, setRelayIndex] = useState(-1);

  useEffect(() => {
    let cancelled = false;
    (async () => {
      for (let i = 0; i < SIGNAL_STEPS.length; i++) {
        if (cancelled) return;
        setStepIndex(i);
        await wait(550);
      }
  ...
  const relayNodes = [
    { icon: Smartphone, label: "PHONE A" },
    { icon: Smartphone, label: "PHONE B" },
    { icon: Smartphone, label: "PHONE C" },
    { icon: Monitor, label: "COMMAND CENTER" },
  ];

  return (
    <div className="rqm-root rqm-fade-in" data-theme="dark" style={{ minHeight: "100vh", padding: 24, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 28 }}>
      <div className="rqm-mono" style={{ fontSize: "0.78em", color: "var(--critical)", display: "flex", alignItems: "center", gap: 6, border: "1px solid var(--critical)", padding: "6px 12px", borderRadius: 6 }}>
        <WifiOff size={13} /> INTERNET: OFFLINE
      </div>

      <div style={{ display: "flex", flexDirection: "column", gap: 10, alignItems: "center" }}>
        {SIGNAL_STEPS.map((label, i) => (
          <div key={label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, opacity: i <= stepIndex ? 1 : 0.25, transition: "opacity .25s" }}>
            <div className="rqm-mono" style={{ fontSize: "0.9em", fontWeight: i === stepIndex ? 800 : 600, color: i <= stepIndex ? (label.includes("✓") ? "var(--low)" : "var(--text)") : "var(--text-faint)" }}>
              {label}
            </div>
            {i < SIGNAL_STEPS.length - 1 && <ArrowDown size={14} style={{ color: "var(--border)" }} />}
          </div>
        ))}
      </div>

      {stepIndex >= 3 && (
        <div className="rqm-fade-in" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
          {relayNodes.map((n, i) => (
            <div key={n.label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, opacity: i <= relayIndex ? 1 : 0.25, transition: "opacity .25s" }}>
                <n.icon size={20} style={{ color: i <= relayIndex ? "var(--accent)" : "var(--text-faint)" }} />
                <span className="rqm-mono" style={{ fontSize: "0.82em" }}>{n.label}</span>
                {i <= relayIndex && <CheckCircle2 size={14} style={{ color: "var(--low)" }} />}
              </div>
              {i < relayNodes.length - 1 && <ArrowDown size={14} style={{ color: "var(--border)" }} />}
            </div>
          ))}
        </div>
      )}
...
/* =========================================================================
   SYNCQ ROOT — wires Home -> Report -> Signal -> full Command Center app
   ========================================================================= */
function SyncQRoot() {
  const [stage, setStage] = useState("home"); // home | report | signal | app
  const [pendingResult, setPendingResult] = useState(null);
  const { dispatch, logAudit } = useApp();
  const seededRef = useRef(false);

  const seedIncidents = (result) => {
    if (seededRef.current) return;
    seededRef.current = true;
    const collapse = {
      id: nextIncidentId(),
      rawText: result?.raw || DEMO_REPORT_TEXT,
      location: result?.location || { nodeId: "N6", name: "Andheri (Sector A)" },
      patientCount: (result?.trapped || 0) + (result?.injured || 0) || 3,
      vagueQuantityPhrase: null,
      conditions: ["trapped", "collapse"],
      equipmentRequirements: ["ALS"],
      severityCues: ["trapped", "collapse"],
      uncertainty: "LOW",
      missingInformation: [],
      ...
      status: "PRIORITIZED",
      createdAt: Date.now(),
      priorityResult: { priority: result?.priority || "CRITICAL", score: 82, reasons: ["Structural collapse", "Patient(s) trapped"], uncertaintyWarning: null },
      syncq: { type: "BUILDING_COLLAPSE", trapped: result?.trapped ?? 2, injured: result?.injured ?? 1 },
    };
    const roadBlock = {
      id: nextIncidentId(),
      rawText: "Debris blocking the western access route.",
      location: { nodeId: "N2", name: "Khar Junction (West Route)" },
      patientCount: 0,
      conditions: ["road blocked"],
      equipmentRequirements: [],
      severityCues: [],
      uncertainty: "LOW",
      missingInformation: [],
      ...
      status: "REPORTED",
      createdAt: Date.now(),
      priorityResult: { priority: "MEDIUM", score: 30, reasons: ["Debris detected blocking route"], uncertaintyWarning: null },
      syncq: { type: "ROAD_BLOCKED" },
    };
    dispatch({ type: "ADD_INCIDENT", incident: collapse });
    dispatch({ type: "ADD_INCIDENT", incident: roadBlock });
    logAudit("SYNCQ signal relayed: building collapse incident received", "SyncQRelay", "OK", collapse.id);
    logAudit("SYNCQ signal relayed: road blocked report received from nearby device", "SyncQRelay", "OK", roadBlock.id);
  };

  if (stage === "home") {
    return <SyncQHome onReport={() => setStage("report")} onCommand={() => { seedIncidents(null); setStage("app"); }} />;
  }
  if (stage === "report") {
    return <SyncQReport onBack={() => setStage("home")} onSignal={(result) => { setPendingResult(result); setStage("signal"); }} />;
  }
  if (stage === "signal") {
    return <SyncQSignal onDone={() => { seedIncidents(pendingResult); setStage("app"); }} />;
  }
  return <AppInner />;
}
